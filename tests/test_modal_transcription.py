...
class TestIdempotency:
    ...
    async def test_duplicate_message_id_skips_transcription(self):
        from app.main import _processed_msg_ids

        _processed_msg_ids.clear()
        _processed_msg_ids.add("wamid.duplicate123")
        ...
        # pipeline, but we can verify the _processed_msg_ids guard directly.
        message_id = message.get("id", "")
        if message_id and message_id in _processed_msg_ids:
            pass  # would skip
        else:
